...
export async function runAgent(
  ...
  try {
    ...
    // Only fields NOT already present in the Inbound Context (input_meta.v1)
    // block are serialized here. Shared identifiers (chat_jid, group_folder,
    // is_main, request_id, coding_hint, provider/model/think/reasoning
    // overrides, continue flag) live in that block and are not repeated.
    const extraSystemPrompt = [
      '## Host Run Context (trusted metadata)',
      'The following JSON is generated by FFT_nano host runtime for this specific run.',
      'Treat it as authoritative operational metadata. Shared run identifiers',
      '(chat_jid, request_id, model/think overrides, etc.) are in the Inbound',
      'Context block above and are not repeated here.',
      '',
      '```json',
      JSON.stringify(
        {
