# FFT_nano vs fft_nano Gap Reference
...
- `fft_nano/fft_nano`: `5196565`

## Why this file exists

This is the persistent reference doc for:
- The 3-repo comparison (`fft_nano`, `nanoclaw`, `fft_nano`)
- The deep gap analysis between `fft_nano` and `fft_nano`
- A practical plan to make `fft_nano` as close as possible to `fft_nano` behavior while preserving farm customizations
...
- `fft_nano`: full gateway platform with many channels, agents, policies, tools, and docs.
...
- `fft_nano`: first-class wizard (`fft_nano onboard`) for workspace, channels, daemon, skills, health.
...
- `fft_nano`: standard workspace set (`AGENTS.md`, `SOUL.md`, `USER.md`, `IDENTITY.md`, `TOOLS.md`, `HEARTBEAT.md`, `BOOTSTRAP.md`, optional `BOOT.md`, `MEMORY.md`, `memory/`).
...
- `fft_nano`: full memory system (`MEMORY.md` + `memory/YYYY-MM-DD.md`, `memory_search`, `memory_get`, compaction memory flush, vector options).
...
- `fft_nano`: first-class heartbeat with targeting, visibility controls, per-agent config, active-hours timezone handling.
...
- `fft_nano`: full cron jobs/wakeups with delivery/session modes and dedicated persisted cron store.
...
## Part 2: Deep Compare (`fft_nano` vs `fft_nano`)

ELI vibe-coder framing:
- `fft_nano` is an assistant operating system.
...
1. No pluggable backend model (fft_nano-style memory backend slot).
2. No embeddings/vector pipeline equivalent.
3. No pre-compaction memory-flush turn equivalent.
4. `memory_get` currently throws when file is missing (fft_nano degrades gracefully).
5. No explicit memory scope policy layer (DM/group/session matching) comparable to fft_nano memory scope config.
...
2. No bootstrap size guardrails equivalent to fft_nano `bootstrapMaxChars` and total cap controls.
3. No per-agent workspace abstraction (single main workspace + group dirs instead).
4. Less formalized workspace drift/doctor checks compared to fft_nano.
...
1. Different persistence shape and operator UX vs fft_nano cron job store/CLI experience.
2. No fft_nano-style defaulting where isolated jobs omit delivery and still default to announce behavior.
3. No deterministic top-of-hour auto-stagger behavior equivalent (stagger is explicit/randomized per run).
4. Less complete runbook/inspection ergonomics than fft_nano cron docs/CLI stack.
...
2. Policy and diagnostics breadth comparable to fft_nano "doctor" style coverage.
...
3. Cron parity pass:
   - Align isolated-job default delivery behavior with fft_nano expectations.
...
2. Add stronger policy scopes and diagnostics to match fft_nano operator ergonomics.
...
To become "as close as possible" to `fft_nano`, the biggest missing blocks are:
