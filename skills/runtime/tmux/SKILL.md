...
metadata:
  fft_nano: '{"emoji":"🧵","os":["darwin","linux"],"requires":{"bins":["tmux"]}}'
---
...
```bash
SOCKET_DIR="${fft_nano_TMUX_SOCKET_DIR:-${CLAWDBOT_TMUX_SOCKET_DIR:-${TMPDIR:-/tmp}/fft_nano-tmux-sockets}}"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/fft_nano.sock"
SESSION=fft_nano-python
...
- Use `fft_nano_TMUX_SOCKET_DIR` (legacy `CLAWDBOT_TMUX_SOCKET_DIR` also supported).
- Default socket path: `"$fft_nano_TMUX_SOCKET_DIR/fft_nano.sock"`.
...
- Scan all sockets: `{baseDir}/scripts/find-sessions.sh --all` (uses `fft_nano_TMUX_SOCKET_DIR`).
