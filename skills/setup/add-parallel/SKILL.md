...
Adds Parallel AI MCP integration to FFT_nano for advanced web research capabilities.
...
- **Non-blocking Design** - Uses FFT_nano scheduler for result polling (no container blocking)

## Prerequisites

User must have:
1. Parallel AI API key from https://platform.parallel.ai
2. FFT_nano already set up and running
3. Runtime available (Docker preferred, or explicit host mode)
...
Add Parallel AI MCP servers after the fft_nano server:
```typescript
const mcpServers: Record<string, any> = {
  fft_nano: ipcMcp
};
...
3. Create a polling scheduled task using `mcp__fft_nano__schedule_task`:
   ```
   ...
   3. Send results to user with mcp__fft_nano__send_message
   4. Use mcp__fft_nano__complete_scheduled_task to mark this task as done
...
- Check runtime logs: `cat groups/main/logs/runtime-*.log | tail -50`
...
- Check task runs: `tail -f logs/fft_nano.log | grep "scheduled task"`
...
5. Restart: `launchctl kickstart -k gui/$(id -u)/com.fft_nano`
